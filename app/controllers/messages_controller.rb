class MessagesController < ApplicationController
  before_action :current_user_must_be_message_sender_user, only: [:edit, :update, :destroy] 
  ...
  def index
    ...
    @messages = @q.result(:distinct => true).includes(:sender_user, :recipient_user).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_message_sender_user
    ...
    def message_params
      params.require(:message).permit(:sender_user_id, :recipient_user_id)
    end
end
